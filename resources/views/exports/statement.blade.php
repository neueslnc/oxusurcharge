...
<div>
    <table>
        <thead>
        <tr>
            <th style="font-size: 15px; width: 20px; text-align: center; padding-top: 10px; padding-bottom: 10px;" colspan="35" >BILDIRGI</th>
        </tr>
        <tr>
            ...
            @for($i = 1; $i <= $days; $i++)
                ...
                @endphp
                <th style="width: 25px; font-weight: bold; border: 5px solid black; border-collapse: collapse; background-color: #fff31e;">{{ $i }}</th>
            @endfor
            <th style="width: 110px; font-weight: bold; border: 5px solid black; border-collapse: collapse;">Itogi</th>
        </tr>
        </thead>
        <tbody>
        ...
        @foreach($teachers as $key => $teacher)
            @php
                ...
                $statics = $teacher->generate_statement($days, $month);
